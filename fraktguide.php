...
class FraktGuide extends CarrierModule {
	...
	if($debug_mode) {
	    ...
	    if (!copy(dirname(__FILE__) . '/img/logo.jpg', _PS_SHIP_IMG_DIR_ . '/' . $carrier->id . '.jpg'))
                return false;
		if($debug_mode) {
    ...
    if($frapostnr) {
		...
		$url = 'https://api.bring.com/shippingguide/products/all.json?clientUrl=insertYourClientUrlHere&weightInGrams=2500&from='.$frapostnr.'&to=5600&product=servicepakke&product=bpakke_dor-dor&product=ekspress09&product=pa_doren&product=a-post&product=b-post&_ga=1.47900642.430994603.1457602699';
		if($debug_mode) {
...
public function getOrderShippingCost($cart, $shipping_cost) {
        ...
        foreach ($productsForShipp as $productForShipp) {
            ...
            $weight = $productForShipp['weight'] * 1000;
            $width = $productForShipp['width'];
            $height = $productForShipp['height'];
            $depth = $productForShipp['depth'];
            
            $url = "http://fraktguide.bring.no/fraktguide/products/$product_id/price.json?from=".Configuration::get('FRAKTGUIDE_FRA_POSTNUMMER')."&to=$address->postcode&weightInGrams=$weight&length=$depth&width=$width&height=$height&edi=".(Configuration::get('FRAKTGUIDE_EDI') ? 'true' : 'false');
            ...
            $item_shipping_cost = $json_obj["Product"]["Price"]["PackagePriceWithoutAdditionalServices"]["AmountWithoutVAT"];
        ...
        if ($orderWeight == $calcWeight) 
        { 
            if($shipping_cost && $forsikring) 
            {
